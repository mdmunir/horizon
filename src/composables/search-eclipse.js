...
// (40) event visibility
  while (((tmp > 0.000001) || (tmp < -0.000001)) && (iter < 50)) {
    ...
    //
    // tmp = sqrt(1 - ((a*v - u*b)/n/l1)^2) * l1 / n
    // tmp = sqrt[(n*l1)^2 - (a*v - u*b)^2]
    // tmp = (u*a + v*b)/n^2 - sqrt[(n*l1)^2 - (a*v - u*b)^2]
    // simplify((u*a + v*b)/n^2 - sqrt(1 - ((a*v - u*b)/n/l1)^2) * l1 / n)
    //
...
function observational(circumstances) {
  ...
  // h = m - lon - deltaT / 13713.44
  // alt = asin(sin(d) * sin(lat) + cos(d) * cos(lat) * cos(h))
