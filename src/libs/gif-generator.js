...
export function generateGIF(element, renderFunction, options = {}) {
    ...
    let current = 0;
    return new Promise(async function addFrame(resolve, reject) {
        ...
        if(options.onProgress){
            options.onProgress(context, current / frames);
        }
        ...
        const _opts = {palette: new Uint32Array(palette), delay: delay};
        writer.addFrame(0, 0, canvas.width, canvas.height, pixels, _opts);
        ...
        if (current < frames) {
            setTimeout(addFrame, 0, resolve, reject);
        } else {
